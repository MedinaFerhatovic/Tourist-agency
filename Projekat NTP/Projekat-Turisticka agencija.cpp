...
struct smjestaj{
    ...
    cout << endl << crta;
        while(true){
          jump:
            ...
            cout << "Da li ste sigurni da zelite sobu sa ID brojem " << rb << ":\n[d/n]->";
            rb=char(rb);
            cin >> dn;
            if(dn=='D' || dn=='d'){
                break;
  ...
  void pretragaSmjestaja(){
    ...
    char dn;
    if(ima==true){
        cout << "\nDa li zelite rezervisati sobu?[d/n]";
        cin >> dn;
        if(dn=='D' || dn=='d'){
            system("cls");
            rezervacija();
    ...
    while(brojac>0){
        ...
        if(userTemp.compare(user)!=0 || pwTemp.compare(pw)!=0){
            if(brojac==1){
                ...
                cout << "Uneseni podaci su pogresni. Pokusajte ponovno.\n";
                cout << "Preostali broj pokusaja: " << --brojac << endl << endl;
            }
        }else{
            return 1;
        }
...
int main(){
    ...
    int izbor1, izbor2, izbor3, izborG;
    smjestaj s;
    registracija r;
    char crtica[]="-----------------------------------------------------";
